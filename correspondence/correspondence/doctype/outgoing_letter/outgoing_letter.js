// Client Script for Outgoing Letter
// Similar functionality to Incoming Letter

frappe.ui.form.on('Outgoing Letter', {
    refresh: function (frm) {
        // Add "View and Assign" button
        if (!frm.is_new()) {
            frm.add_custom_button(__('View and Assign'), function () {
                show_view_and_assign_dialog_outgoing(frm);
            }, __('Actions'));

            // Add "Export to Word" button
            frm.add_custom_button(__('Export to Word'), function () {
                frappe.call({
                    method: 'correspondence.correspondence.doctype.outgoing_letter.outgoing_letter.generate_word_document',
                    args: {
                    ...
                    callback: function (r) {
                        if (r.message) {
                            // Download the file
                            download_file_outgoing(r.message.filename, r.message.content);
                        }
                    }
                });
            }, __('Actions'));
    ...
    status: function (frm) {
        ...
        if (frm.doc.status === 'Sent' && !frm.doc.date_sent) {
            frm.set_value('date_sent', frappe.datetime.get_today());
        }
    }
});
...
function add_custom_buttons(frm) {
    // Refresh Related Documents button (available even for new docs)
    ...
    if (!frm.is_new()) {
        ...
        // Create Transmittal
        frm.add_custom_button(__('Create Transmittal'), function () {
            create_transmittal(frm);
        }, __('Create'));
...
function show_voice_to_text_dialog(frm) {
            ...
            },
            {
                ...
                options: `
                    <div class="recording-controls" style="margin: 15px 0;">
                        <button class="btn btn-primary btn-sm" id="start-recording-out">
                            <i class="fa fa-microphone"></i> ${__('Start Recording')}
                        </button>
                        <button class="btn btn-danger btn-sm" id="stop-recording-out" style="display: none;">
                            <i class="fa fa-stop"></i> ${__('Stop Recording')}
                        </button>
                        <div id="recording-status-out" style="margin-top: 10px; display: none;">
                            <span class="indicator red"></span>
                            <span>${__('Recording...')}</span>
                            <span id="recording-timer-out">00:00</span>
                        </div>
                        <audio id="audio-preview-out" controls style="width: 100%; margin-top: 10px; display: none;"></audio>
                    </div>
                `
            },
            {
                ...
                xhr.onload = function () {
                    if (xhr.status === 200) {
                        ...
                        if (response.message) {
                            ...
                            convertAudioOutgoing(audioFile, values, frm, d);
                        }
                    } else {
                        frappe.msgprint(__('Upload failed'));
                    }
    ...
    setTimeout(() => {
        let $startBtn = d.$wrapper.find('#start-recording-out');
        let $stopBtn = d.$wrapper.find('#stop-recording-out');
        let $status = d.$wrapper.find('#recording-status-out');
        let $preview = d.$wrapper.find('#audio-preview-out');
        let $timer = d.$wrapper.find('#recording-timer-out');
        let startTime = null;
...
function convertAudioOutgoing(audioFile, values, frm, dialog) {
    frappe.call({
...
function create_transmittal(frm) {
    frappe.model.with_doctype('Document Transmittal', function () {
        let transmittal = frappe.model.get_new_doc('Document Transmittal');

        // Pre-fill details
        transmittal.recipient = frm.doc.recipient; // Assuming recipient is a User link, might need mapping if it's just text
        transmittal.reason_for_issue = 'For Information';

        // Add item
        let item = frappe.model.add_child(transmittal, 'transmittal_items');
        item.document_type = frm.doctype;
        item.document_name = frm.docname;
        item.title = frm.doc.subject;

        frappe.set_route('Form', 'Document Transmittal', transmittal.name);
    });
}
...
function show_related_documents_panel(frm) {
    if (frm.doc.related_documents && frm.doc.related_documents.length > 0) {
        ...
        if (frm.fields_dict['related_documents_section'].$wrapper.find('.related-documents-panel').length > 0) {
            frm.fields_dict['related_documents_section'].$wrapper.find('.related-documents-panel').remove();
        }
...
// Function to show View and Assign Dialog for Outgoing Letters
function show_view_and_assign_dialog_outgoing(frm) {
    let d = new frappe.ui.Dialog({
        ...
        fields: [
            {
                ...
                options: get_letter_details_html_outgoing(frm)
            },
            {
            ...
            frm.save().then(() => {
                ...
                frappe.call({
                    ...
                    args: {
                        ...
                        title: frm.doc.subject || __('Outgoing Letter Assignment')
                    },
                    callback: function (r) {
                        if (!r.exc) {
                            frappe.show_alert({
...
// Helper function to generate letter details HTML for Outgoing Letters
function get_letter_details_html_outgoing(frm) {
    let doc = frm.doc;
        ...
        <div style="padding: 15px; background: #f9f9f9; border-radius: 8px; margin-bottom: 15px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div>
                    ...
                    <p style="margin: 5px 0;">
                        <strong>${__('Recipient Organization')}:</strong> 
                        ${doc.recipient_organization || '-'}
                    </p>
                    <p style="margin: 5px 0;">
                        <strong>${__('Date Created')}:</strong> 
                        ${frappe.datetime.str_to_user(doc.date_created) || '-'}
                    </p>
                    ${doc.date_sent ? `
                    <p style="margin: 5px 0;">
                        <strong>${__('Date Sent')}:</strong> 
                        ${frappe.datetime.str_to_user(doc.date_sent)}
                    </p>
                    ` : ''}
                </div>
                <div>
                    ...
                    ${doc.related_incoming_letter ? `
                    <p style="margin: 5px 0;">
                        <strong>${__('Related Incoming Letter')}:</strong> 
                        <a href="/app/incoming-letter/${doc.related_incoming_letter}" target="_blank" style="color: #2490ef;">
                            ${doc.related_incoming_letter}
                        </a>
                    </p>
                    ` : ''}
...
function download_file_outgoing(filename, content) {
